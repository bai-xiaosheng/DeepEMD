...
class DeepEMD(nn.Module):
    ...
    def __init__(self, args, mode='meta'):
        ...
        if self.mode == 'pre_train':
            self.fc = nn.Linear(8, self.args.num_class)   #常规网络训练
    ...
    def pre_train_forward(self, input):
        # a = self.encode(input, dense=False).squeeze(-1)
        # a = a.squeeze(-1)
    ...
    def get_weight_vector(self, A, B):
        ...
        B = F.adaptive_avg_pool2d(B, [1, 1])  #均值
    ...
    def emd_forward_1shot(self, proto, query):
        proto = proto.squeeze(0)  #第一维度为1时，删除第一维度

        weight_1 = self.get_weight_vector(query, proto)  #45，3，9
        weight_2 = self.get_weight_vector(proto, query)

        proto = self.normalize_feature(proto)
        query = self.normalize_feature(query)
        # 找相似度
        similarity_map = self.get_similiarity_map(proto, query)  #45，3，9，9
    ...
    def get_sfc(self, support):
        ...
        # init the proto
        SFC = support.view(self.args.shot, -1, 8, support.shape[-2], support.shape[-1]).mean(dim=0).clone().detach()
        ...
        label_shot = torch.arange(self.args.way).view(self.args.way,1).repeat((1,self.args.shot)).view(-1)
    ...
    def get_emd_distance(self, similarity_map, weight_1, weight_2, solver='opencv'):
        num_query = similarity_map.shape[0]  #45
        num_proto = similarity_map.shape[1]  #3
        num_node=weight_1.shape[-1]  #9
        ...
        if solver == 'opencv':  # use openCV solver
            ...
            weight_2 = weight_2.permute(1, 0, 2)   #交换数组维度
    ...
    def get_similiarity_map(self, proto, query):
        ...
        query = query.view(query.shape[0], query.shape[1], -1)   #45,640,9
        proto = proto.view(proto.shape[0], proto.shape[1], -1)   #3,640,9

        proto = proto.unsqueeze(0).repeat([num_query, 1, 1, 1])  #45,3,640,9
        query = query.unsqueeze(1).repeat([1, way, 1, 1])    #45,3,640,9
        proto = proto.permute(0, 1, 3, 2)  #交换位置  45，3，9，640 3个类，每类选取640个特征，每个特征长9，重复45次
        query = query.permute(0, 1, 3, 2)  #45，3，9，640  总共45个样本，每个样本取640个特征，每个特征长9，重复三次
        feature_size = proto.shape[-2]   #9

        if self.args.metric == 'cosine':
            proto = proto.unsqueeze(-3)  #45，3，1，9，640
            query = query.unsqueeze(-2)  #45，3，9，1，640
            query = query.repeat(1, 1, 1, feature_size, 1) #45，3，9，9，640
            '''
            dim = -1,按行计算
            首先有45行，各取出一行，proto：3，1，9，640；query：3，9，9，640
            然后在取行，proto：1，9，640；query：9，9，640
            继续取行，proto：9，640；query;9,640,计算余弦相似度 9，这个是proto特征向量（9，640）与query特征向量（1，640）进行计算
            将proto重复9次，与query九个特征向量匹配：9，9；每一行代表query一个特征向量的相似度，每一列代表proto特征向量的相似度
            往回推，similarity_map：45，3，9，9
            '''
            similarity_map = F.cosine_similarity(proto, query, dim=-1)  #45,3,9,9
